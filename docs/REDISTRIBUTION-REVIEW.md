...
| SP11 runtime-idle suspend guard | `rootfs/usr/local/libexec/sp11-runtime-idle-suspend-guard` and systemd drop-in | independently developed and qualified on the tested hardware | REVIEWED ORIGINAL WORK, MIT | Preserve fail-closed ordering, conservative no-opt-in fallback, and suspend qualification record |
...
| Minimal GPU/Wi-Fi/Bluetooth/QUP firmware allowlist and derived WCN7850 board record | `firmware/allowlist.tsv` and `firmware/derived.tsv`; no blobs tracked | linux-firmware tag `20260622`, commit `b2722d241309a1872446c1d00c2e812bad055f89`, plus signed wireless-regdb package | REVIEWED REDISTRIBUTABLE under file-specific WHENCE terms | Extract only exact allowlisted hashes and the named unmodified board record from signed packages; retain every named license/notice |
| Surface ADSP/CDSP images and GPU secure image | local machine only; hashes recognized by `firmware/denylist.tsv` | external foundation/Windows-derived workflow; absent from upstream linux-firmware | EXTERNAL PROPRIETARY PREREQUISITE | Never publish; permit only explicit local operator import and clearly report missing live hardware |
| hexagonrpc and libssc sensor patches | `userspace/hexagonrpc`, `userspace/libssc` | pinned GPL-3.0-or-later upstream projects plus local patches | REVIEWED OPEN-SOURCE ADAPTATION | Preserve exact source identities, patches, notices, and corresponding source for binaries |
| Surface sensor configuration and registry | local machine only; not included | official Microsoft driver pack and locally generated persistence | EXTERNAL PROPRIETARY PREREQUISITE | Never publish or place in a source/binary payload without an independent redistribution grant |
