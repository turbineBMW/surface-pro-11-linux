...
| Minimal GPU/Wi-Fi/Bluetooth/QUP firmware allowlist | `firmware/allowlist.tsv`; no blobs tracked | linux-firmware tag `20260622`, commit `b2722d241309a1872446c1d00c2e812bad055f89`, plus signed wireless-regdb package | REVIEWED REDISTRIBUTABLE under file-specific WHENCE terms | Extract only exact allowlisted hashes from signed packages and retain every named license/notice |
| Surface ADSP/CDSP images, audio topology, and ath12k `board.bin` | local machine only; hashes recognized by `firmware/denylist.tsv` | external foundation/Windows-derived workflow; absent from upstream linux-firmware | EXTERNAL PROPRIETARY PREREQUISITE | Never publish; permit only explicit local operator import and clearly report missing live hardware |
