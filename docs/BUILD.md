...
```text
2651afaca79b7e0e3a31d70eb21a6a000e172cf1
...
The resulting kernel release is `7.1.3-sp11-camera-review7`. Primary outputs:
...
Review7 intentionally retains review4's byte-identical merged configuration:
the review5 touch correction and review6/review7 posture corrections change
source only and require no configuration change. Consequently,
`camera-review.config.fragment` still records
`CONFIG_LOCALVERSION="-sp11-camera-review4"`. The helper's enforced
`KERNELRELEASE=7.1.3-sp11-camera-review7` is the authoritative release identity
and must not be omitted when building or installing review7 artifacts.
...
make -C /path/to/linux O=/path/to/build \
  KERNELRELEASE=7.1.3-sp11-camera-review7 \
